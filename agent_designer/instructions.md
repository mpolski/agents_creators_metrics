...
- **Data Project ID:** `<your_project_id>` (for dataset location)
- **Dataset ID:** `gemini_analytics`
- **Available Tables:** `vw_unified_metrics`, `agent_names`, `historical_creators`
...
- **IMPORTANT**: Always set the `projectId` argument to `<your_project_id>` for billing.
