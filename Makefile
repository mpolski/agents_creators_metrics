...
playground:
	...
	@echo "|                                                                             |"
	@echo "| 🔍 IMPORTANT: Select the 'adk_agent' folder to interact with your agent.          |"
	@echo "==============================================================================="
...
deploy:
	...
	(uv export --no-hashes --no-header --no-dev --no-emit-project --no-annotate > adk_agent/app_utils/.requirements.txt 2>/dev/null || \
	uv export --no-hashes --no-header --no-dev --no-emit-project > adk_agent/app_utils/.requirements.txt) && \
	uv run -m adk_agent.app_utils.deploy \
		--source-packages=./adk_agent \
		--entrypoint-module=adk_agent.agent_engine_app \
		--entrypoint-object=agent_engine \
		--requirements-file=adk_agent/app_utils/.requirements.txt \
...
eval:
	...
	uv run adk eval ./adk_agent $${EVALSET:-tests/eval/evalsets/basic.evalset.json} \
